...
function roleAssessmentDetail(row, classification) {
  let detail = {};
  try {
    detail = row.reason_detail ? JSON.parse(row.reason_detail) : {};
  } catch {
    detail = {};
  }
...
async function refreshArticleProcessingStatus(db, articleId, sourceContentHash) {
  const counts = await db.prepare(
    `SELECT
  ...
  let status = "phase_a_processed";
  if (Number(counts?.relevant || 0) > 0) status = "phase_a_ready";
  else if (Number(counts?.needs_review || 0) > 0) status = "phase_a_review";
  else if (Number(counts?.rejected || 0) > 0) status = "phase_a_rejected";

  await db.prepare("UPDATE raw_articles SET processing_status = ? WHERE id = ?")
    .bind(status, articleId).run();
}

async function loadRoleClassifierCandidates(db, limit) {
  const { results } = await db.prepare(
    `SELECT
       a.article_id,
       a.club_id,
...
export async function drainRoleClassifier(db, env, options = {}) {
  const maxItems = Math.max(1, Math.min(50, Number(options.maxItems || 10)));
  const maxDurationMs = Math.max(5000, Number(options.maxDurationMs || 45000));
  const interRequestDelayMs = Math.max(0, Number(options.interRequestDelayMs ?? 4000));
  const startedAtMs = Date.now();
  const rows = await loadRoleClassifierCandidates(db, maxItems);
  ...
  return {
    ...
    started_at: new Date(startedAtMs).toISOString(),
    finished_at: new Date().toISOString(),
    inter_request_delay_ms: interRequestDelayMs,
    max_duration_ms: maxDurationMs,
    stop_reason: stopReason,
    ...totals,
    examples
